...
本模板维护项目规则、文档结构和通用协作配置。开发环境由各项目自行选择和维护。

修改模板结构或规则后，检查占位内容与文档入口，并运行格式检查：
...
- [ ] 补齐开发环境及安装、编译、运行、测试、打包、发布命令。
- [ ] 声明产物保存位置、校验方式和发布、部署、回滚入口。
...
派生项目必须把这些命令补成可直接执行的项目真值源，说明运行位置、前置条件，以及是否依赖硬件、外部服务或网络。

## 环境要求

- Git
- TODO：所需工具链、SDK、系统依赖及安装方式
- TODO：硬件、签名或外部服务条件

## 开发环境

TODO：说明如何准备开发环境、安装依赖，以及如何与 CI 保持一致。
...
|------|------|----------|------------------------|----------|
| 开发 | TODO | TODO：开发环境 | TODO | TODO |
| CI | TODO | TODO：GitHub Actions / GitLab CI / self-hosted runner | TODO | TODO |
| 仿真/模拟器 | TODO：例如 QEMU、浏览器、mock 服务 | TODO：开发环境 / CI | TODO | TODO |
| 无硬件运行 | TODO：验证配置、线程编排或基础流程 | TODO：开发环境 / CI | 否 | TODO |
| 硬件诊断 | TODO：验证设备、总线、传感器或产线夹具 | TODO | 是 | TODO |
| 生产/部署 | TODO：打包、发布、安装、升级、回滚 | TODO：CI / self-hosted runner / 产线环境 | TODO | TODO |
...
本节必须写清楚产物的生成环境、保存路径、名称和校验方式，以及发布、部署、验收、回滚如何使用同一份产物。
